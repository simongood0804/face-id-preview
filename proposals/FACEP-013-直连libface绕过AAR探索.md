...
> **更新（2026-08-24）**：本提案探索时 AAR 的地标模型为 `2d106det`（**106 点**，本文中所有"106 轮廓点"均为当时的 AAR 状态）。v3.3 起 AAR 已升级为 `pipnet68`（**PIPNet 68 点**），但**"绕过 AAR 直连 `libface.so` 不可行"的结论不变**——加载障碍来自 linker namespace 隔离与 `face_vision_init` 崩溃（§4），与点位数量无关。106→68 的对接细节见 FACEP-010 §2.5 / 设计方案 §18.4。
