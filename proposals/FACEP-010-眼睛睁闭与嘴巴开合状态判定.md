...
即：判定逻辑**只依赖"语义区域"（如"左眼上眼睑""下唇中央"），不直接出现 `35/39` 这类模型索引**。模型索引通过一张**可配置的映射表**（`LandmarkIndexMapping`）与语义区域解耦。后续若更换算法/点位定义（如换 68 点、5 点，或更换模型），**只需改这张映射表，判定逻辑与渲染层零改动**。
...
**几何判据（EAR，Eye Aspect Ratio）**——经典且与姿态无关性较好的方法：

1. 经映射取**左右眼的上下眼睑点集**与**左右眼角**；
2. 上下眼睑点**按对应列**计算纵向距离均值（见 §2.4 对应关系）；
3. 除以眼角水平距离，得到单眼 EAR：

```
EAR = avg( |上睑[i].y − 下睑[i].y| )  /  (|眼尾.x − 眼角.x|)
```

- 睁眼时 EAR 接近正常值（约 0.2~0.3）；
- 闭眼时 EAR 显著变小（趋近 0.1 以下）。

建议实现：
1. 分别计算左眼、右眼 EAR；
2. 双眼 EAR 均值 < 阈值（如 `0.18`）判为 **闭眼**，否则 **睁眼**；
3. `eyeOpenRatio = EAR / 基准EAR`（基准可在初始化/前若干帧动态校准）。

> 可选增强：项目已有 `pfld_eye_int8.dlc`（眼睑模型，见 `REQUIRED_MODEL_FILES`）。若纯几何 EAR 在强侧脸/遮挡下不稳，可在后续阶段切换为由 `pfld_eye` 回归输出开合度，几何法作为 fallback。
...
> 即使只输出"睁眼/闭眼/张嘴/闭嘴"四个基础状态，单帧 EAR/MAR 的布尔判断（§3.5/§3.6）仍存在两个**直接影响正确性**的工程问题：
> 1. **闭眼时上下眼睑点位不会完全重合**——模型在 106 点定位时，闭眼状态下上睑点与下睑点仍有**最小残差距离**（不会都叠到 0），若只用"EAR < 阈值"布尔判断，阈值与残差边界抖动会产生**闪变（flicker）**，导致闭眼/睁眼状态在阈值附近来回跳；
...
data class EyeMouthFrameInput(
    ...
    val eyeOpenRatio: Float,     // 本帧眼睛开合度 0~1（EAR 归一化）
...
| 阈值 | 含义 |
|------|------|
| `CLOSE_RATIO` | 开合度 ≤ 此值（如 `0.18`）判定进入"闭眼候选" |
| `OPEN_RATIO` | 开合度 ≥ 此值（如 `0.35`）判定回到"睁眼候选" |
| （两者之间为**滞回区间**，状态保持不变） | 吸收残差抖动 |

即：
- 只有开合度**降到 `CLOSE_RATIO` 以下**才启动"闭眼累计"；
- 只有开合度**升到 `OPEN_RATIO` 以上**才清除闭眼状态；
- 在 `CLOSE_RATIO ~ OPEN_RATIO` 区间内（如 0.18~0.35），**维持上一状态**，不会因残差在阈值附近跳动而闪变。

**方案 B（正式设计）：动态基准校准** —— 残差会随人头距摄像头远近、姿态而变化，静态阈值可能误判。**完整方案见 §3.7.6 `EyeMouthCalibrator`**：维护每个驾驶员的"睁/闭眼、张/闭嘴"基准（`REF_EYE_OPEN_EAR` 等），动态换算滞回阈值，并由**驾驶门开关信号**触发换人复位重校。核心思路：
```
CLOSE_RATIO = REF_CLOSED + (REF_OPEN - REF_CLOSED) * 0.35
...
> - **每人眼/嘴大小不同**——EAR/MAR 的绝对范围因人而异，同一阈值下 A 的全睁眼 EAR 可能小于 B 的全闭眼 EAR；
> - **距摄像头距离 / 姿态 / 光照变化**——同一人 EAR/MAR 也会整体漂移；
...
| 基准量 | 含义 | 来源 |
|--------|------|------|
| `REF_EYE_OPEN_EAR` | 该驾驶员"睁眼"时 EAR 的典型值 | 校准窗口内睁眼帧中位数 |
| `REF_EYE_CLOSED_EAR` | 该驾驶员"闭眼"时 EAR 的典型值（残差） | 校准/运行中闭眼帧中位数（或按开放比例推导） |
...
```
CLOSE_RATIO  = REF_EYE_CLOSED_EAR + (REF_EYE_OPEN_EAR - REF_EYE_CLOSED_EAR) * 0.35   // 滞回下界
OPEN_RATIO   = REF_EYE_CLOSED_EAR + (REF_EYE_OPEN_EAR - REF_EYE_CLOSED_EAR) * 0.70   // 滞回上界
```
...
- **重校窗口**（门开后人脸重新稳定的一段时间，如 3~5s）内，采集新驾驶员基准确认：
  - 假定新驾驶员在窗口内处于**正常驾驶状态（睁眼、闭嘴）**，采集该段 `EAR`/`MAR` 的高位区（睁眼 EAR、闭嘴 MAR）作为 `REF_EYE_OPEN_EAR` / `REF_MOUTH_CLOSED_MAR`；
  - `REF_EYE_CLOSED_EAR` / `REF_MOUTH_OPEN_MAR` 若无法在窗口强制采集，则用**经验比例**（如闭眼残差 ≈ 睁眼基准的 40%）、或由后续运行中的动态跟踪补齐；
- **门关信号 + 车速上电** → 确认驾驶员已就位，锁定校准结果，进入运行状态。

**路径 2：运行中持续自适应（滑动窗口分位数）**

- 不依赖门信号时，校准器在运行中持续跟踪 `EAR`/`MAR` 的滑动分布：
  - 维护**滑动窗口**（如最近 300 帧）的 `EAR` 分位数：取 **P90（高位）= 睁眼 EAR 候选**、**P10（低位）= 闭眼 EAR 候选**；
  - 用**移动平均/EWMA** 缓慢更新 `REF_EYE_OPEN_EAR` / `REF_EYE_CLOSED_EAR`（嘴巴同理）；
...
FaceSDK.infer()
  ...
  → [新增] EyeMouthStateEstimator（单帧几何计算）
       ...
       ├─ 从 106 点提取 眼睛/嘴巴 区域点
       └─ 计算 EAR / MAR → 输出连续开合度 eyeOpenRatio / mouthOpenRatio
...
| 3 | `algorithm/EyeMouthStateEstimator.kt` | 新增 | 单帧几何判定器：依赖 `LandmarkRegion` + 注入 `LandmarkIndexMapping`，计算 EAR/MAR，输出连续开合度 `eyeOpenRatio`/`mouthOpenRatio`。纯计算、阈值可配，便于单测 |
...
- 新建 `EyeMouthStateEstimator`，依赖 `LandmarkRegion`、注入 `LandmarkIndexMapping`，实现 EAR/MAR 计算 + 连续开合度输出；
...
| 新增计算增加算法耗时 | 性能 | EAR/MAR 仅涉及十几点的简单距离运算，开销可忽略（<0.1ms）；校准为滑动统计，无显著开销 |
...
本提案聚焦**基础状态判定的正确性**：在算法模块内部，基于已输出的 106 点中**眼睛、嘴巴区域点位**，通过 **EAR / MAR 几何判据**增加一步推理，**先只输出"睁眼 / 闭眼 / 张嘴 / 闭嘴"四个基础状态**（及可选开合度），作为 `FaceIDResult` 新增字段透出。**暂不涉及**疲劳（闭眼持续时长）、打哈欠/说话等上层业务判定，待基础状态稳定正确后再行扩展。

**关键设计（围绕基础状态正确性）**：
1. **映射解耦**：判定逻辑通过 `LandmarkRegion` 语义区域 + `LandmarkIndexMapping` 索引映射解耦，不写死任何模型索引，后续更换算法只需改映射表；
2. **单帧几何 + 状态防抖**：`EyeMouthStateEstimator`（单帧 EAR/MAR 出连续开合度）→ `EyeMouthStateMachine`（短确认防抖，对齐 `DistractionStateMachine`，不含业务判定）→ 稳定基础状态；
